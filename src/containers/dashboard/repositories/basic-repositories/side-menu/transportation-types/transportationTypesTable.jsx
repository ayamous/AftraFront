...
function TransportationTypesTable(props) {
  const {
  ...
  const { isLoading: removing, mutate: remove } = useMutation(
    (id) => deleteTransportationType(id),
    {
      onSuccess: () => onSuccess(t("Type de transportation suprimée")),
      onError: () => operationFailed(t)
    }
  );
  const { isLoading: updating, mutate: update } = useMutation(
    (id) => editTransportationType(id, values),
    {
      onSuccess: () => {
        onSuccess(t("Type de transportation modifiée"));
        setEdit(-1);
      },
      onError: () => operationFailed(t)
    }
  ...
  const columns = [
    {
      title: t("Code du type de transportation"),
      dataIndex: "code",
      editable: true,
      render: (text, { id }) => {
...
TransportationTypesTable.propTypes = {
...
TransportationTypesTable.defaultProps = {
  data: null
};

export default TransportationTypesTable;
