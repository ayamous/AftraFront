...
function CountryGroupsTable(props) {
  ...
  const { isLoading: removing, mutate: remove } = useMutation(
    (id) => deleteCountriesGroup(id),
    {
      onSuccess: () => onSuccess(t("Groupement de pays suprimée")),
      onError: () => operationFailed(t)
    }
  );
  const { isLoading: updating, mutate: update } = useMutation(
    (id) => editCountriesGroup(id, values),
    {
      onSuccess: () => {
        onSuccess(t("Groupement de pays modifiée"));
        setEdit(-1);
      },
      onError: () => operationFailed(t)
    }
  ...
  const columns = [
    {
      title: t("Référence groupement de pays"),
      dataIndex: "reference",
      editable: true,
      render: (text, { id }) => {
        ...
        return (
          <Input
            defaultValue={text}
            onChange={(e) => { values.reference = e.target.value; }}
          />
        );
      }
    },
    {
      title: t("Code groupement"),
      dataIndex: "code",
      editable: true,
      render: (text, { id }) => {
        const editRow = edit === id;
        ...
        return (
          <Input
            defaultValue={text}
            onChange={(e) => { values.code = e.target.value; }}
          />
        );
      }
    },
...
CountryGroupsTable.defaultProps = {
  data: null
};

export default CountryGroupsTable;
